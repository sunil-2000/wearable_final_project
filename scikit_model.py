...
class Model:
    ...
    def k_fold_validation(self, model):
        ...
        scores = cross_val_score(kfold_validated, self.X, self.Y, scoring='accuracy', cv=kf, n_jobs=-1)
        print("accuracy scores: {}".format(list(map(lambda x: round(x*100,2),scores))))
        ...
        print("Total labels in train:{}, +1 labels:{}, -1 label:{}".format(len(yTr),np.count_nonzero(yTr[yTr > 0]),np.count_nonzero(yTr[yTr < 0])))
        print("Total labels in test:{}, +1 labels:{}, -1 label:{}".format(len(yTe),np.count_nonzero(yTe[yTe > 0]),np.count_nonzero(yTe[yTe < 0])))
    ...
    def knn_k_plot(self):
        ...
        k_lst = []
        xTr, xTe, yTr, yTe = train_test_split(self.X, self.Y, random_state=42 ) # 50% of each label per set
